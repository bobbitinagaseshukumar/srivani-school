...
export default function AdminPortal() {
  const { 
    ...
    campusFacilities, editCampusFacility, addCampusFacility, deleteCampusFacility,
    logoutUser 
  ...
  const [studentForm, setStudentForm] = useState({
    name: '', registerNo: '', phone: '', address: '', class: 'Class 10', section: 'A',
    email: '', password: '', dob: '2010-01-01', gender: 'Male', parentName: '', parentPhone: '', parentEmail: '',
  ...
  const [facilityForm, setFacilityForm] = useState({ title: '', icon: 'Book', cap: '', desc: '', features: '' });
        ...
        <div className="glassmorphism p-5 rounded-3xl border border-white/50 shadow-md">
          ...
          <nav className="flex flex-col gap-1.5 text-xs font-semibold">
            {[
              ...
              { id: 'Students', icon: Users, label: 'Enroll Students' },
              { id: 'Facilities', icon: Layers, label: 'Campus Facilities' },
        ...
        {activeTab === 'Students' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-2xl font-extrabold font-montserrat">Student Enrollment</h2>
                <p className="text-xs text-slate-400 font-light mt-1">Enroll students with Registration Number as login ID. Parent portal uses the same password.</p>
              </div>
              <button
              ...
              >
                <Plus size={14} /> {showStudentForm ? 'Close Form' : 'Enroll Student'}
              </button>
            </div>
            ...
            {showStudentForm && (
              <form
                onSubmit={(e) => {
                  ...
                  alert(`Student enrolled. Login ID: ${studentForm.registerNo} | Password: ${studentForm.password}`);
                ...
                className="glassmorphism p-5 rounded-2xl border border-white/40 shadow-lg space-y-4 max-w-3xl"
              >
                <div className="grid sm:grid-cols-2 gap-4">
                  <input required placeholder="Student Full Name" value={studentForm.name} onChange={e => setStudentForm(p => ({ ...p, name: e.target.value }))} className="px-3 py-2 border rounded-xl bg-white/70 dark:bg-slate-900/50 text-xs" />
                  <input required placeholder="Registration Number (Login ID)" value={studentForm.registerNo} onChange={e => setStudentForm(p => ({ ...p, registerNo: e.target.value }))} className="px-3 py-2 border rounded-xl bg-white/70 dark:bg-slate-900/50 text-xs" />
                  <input required type="password" placeholder="Password (shared with parent)" value={studentForm.password} onChange={e => setStudentForm(p => ({ ...p, password: e.target.value }))} className="px-3 py-2 border rounded-xl bg-white/70 dark:bg-slate-900/50 text-xs" />
                  <select value={studentForm.class} onChange={e => setStudentForm(p => ({ ...p, class: e.target.value }))} className="px-3 py-2 border rounded-xl bg-white/70 dark:bg-slate-900/50 text-xs">
                    {classes.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                  <select value={studentForm.section} onChange={e => setStudentForm(p => ({ ...p, section: e.target.value }))} className="px-3 py-2 border rounded-xl bg-white/70 dark:bg-slate-900/50 text-xs">
                    {sections.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                  <input required placeholder="Student Email" value={studentForm.email} onChange={e => setStudentForm(p => ({ ...p, email: e.target.value }))} className="px-3 py-2 border rounded-xl bg-white/70 dark:bg-slate-900/50 text-xs" />
                  <input required placeholder="Parent / Guardian Name" value={studentForm.parentName} onChange={e => setStudentForm(p => ({ ...p, parentName: e.target.value }))} className="px-3 py-2 border rounded-xl bg-white/70 dark:bg-slate-900/50 text-xs" />
                  <input required placeholder="Parent Phone" value={studentForm.parentPhone} onChange={e => setStudentForm(p => ({ ...p, parentPhone: e.target.value, phone: e.target.value }))} className="px-3 py-2 border rounded-xl bg-white/70 dark:bg-slate-900/50 text-xs" />
                  <input placeholder="Parent Email" value={studentForm.parentEmail} onChange={e => setStudentForm(p => ({ ...p, parentEmail: e.target.value }))} className="px-3 py-2 border rounded-xl bg-white/70 dark:bg-slate-900/50 text-xs" />
                </div>
                <input placeholder="Address" value={studentForm.address} onChange={e => setStudentForm(p => ({ ...p, address: e.target.value }))} className="w-full px-3 py-2 border rounded-xl bg-white/70 dark:bg-slate-900/50 text-xs" />
                <button type="submit" className="bg-emerald-600 text-white font-bold text-xs px-5 py-2.5 rounded-xl">Enroll Student & Create Parent Account</button>
              </form>
            )}
            ...
            <div className="flex flex-wrap gap-3 items-center">
              ...
              <select value={classFilter} onChange={e => setClassFilter(e.target.value)} className="px-3 py-2 border rounded-xl bg-white/70 dark:bg-slate-900/50 text-xs">
                ...
                <table className="w-full text-xs text-left">
                  <thead>
                    <tr className="border-b border-slate-200 dark:border-slate-850 bg-slate-50 dark:bg-slate-900/40 text-slate-400 font-bold uppercase tracking-wider text-[9px]">
                      ...
                      <th className="p-4">Registration No (Login ID)</th>
        ...
        {activeTab === 'Facilities' && (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-xs text-slate-400 font-light mt-1">Update the public Campus Facilities page content.</p>
            </div>
            <div className="grid gap-4">
              {(campusFacilities || []).map(fac => (
                <div key={fac.id} className="glassmorphism p-4 rounded-2xl border border-white/40 space-y-2">
                  <div className="flex justify-between items-start gap-2">
                    <div>
                      <h4 className="font-bold text-sm">{fac.title}</h4>
                      <p className="text-[10px] text-slate-500">{fac.desc}</p>
                    </div>
                    <button
                      onClick={() => {
                        setFacilityEditId(fac.id);
                        setFacilityForm({ title: fac.title, icon: fac.icon, cap: fac.cap, desc: fac.desc, features: (fac.features || []).join(', ') });
                      }}
            ...
            {facilityEditId && (
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  editCampusFacility(facilityEditId, {
                    ...facilityForm,
                ...
                className="glassmorphism p-5 rounded-2xl border border-white/40 space-y-3 max-w-xl"
              >
                <input required value={facilityForm.title} onChange={e => setFacilityForm(p => ({ ...p, title: e.target.value }))} className="w-full px-3 py-2 border rounded-xl text-xs" placeholder="Title" />
                <input required value={facilityForm.cap} onChange={e => setFacilityForm(p => ({ ...p, cap: e.target.value }))} className="w-full px-3 py-2 border rounded-xl text-xs" placeholder="Capacity" />
                <textarea required rows={3} value={facilityForm.desc} onChange={e => setFacilityForm(p => ({ ...p, desc: e.target.value }))} className="w-full px-3 py-2 border rounded-xl text-xs" placeholder="Description" />
                <input value={facilityForm.features} onChange={e => setFacilityForm(p => ({ ...p, features: e.target.value }))} className="w-full px-3 py-2 border rounded-xl text-xs" placeholder="Features (comma separated)" />
                <div className="flex gap-2">
                  <button type="submit" className="bg-blue-600 text-white text-xs font-bold px-4 py-2 rounded-xl">Save</button>
                  <button type="button" onClick={() => setFacilityEditId(null)} className="text-xs px-4 py-2 rounded-xl bg-slate-100">Cancel</button>
                </div>
              </form>
            )}
          </div>
