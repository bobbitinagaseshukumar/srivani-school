...
export default function Faculty() {
  const { teachers, subjects, departments } = useContext(AppContext);
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12 space-y-8 text-slate-800 dark:text-slate-100 text-left">
      ...
      {/* Subject Quick Filter Badges */}
      <div className="flex flex-wrap gap-2 justify-center">
        <button
          onClick={() => { setSubjectFilter(''); setDeptFilter(''); }}
          className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${!subjectFilter ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-blue-50 dark:hover:bg-slate-700'}`}
        >
          All Subjects
        </button>
        {(subjects || []).map((sub) => (
          <button
            key={sub.id || sub.code}
            onClick={() => setSubjectFilter(sub.code)}
            className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${subjectFilter === sub.code ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-blue-50 dark:hover:bg-slate-700'}`}
          >
            {sub.name}
          </button>
        ))}
