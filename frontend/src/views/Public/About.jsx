...
export default function About() {
  const { schoolInfo, managementCommittee, teachers, marks } = useContext(AppContext);
    ...
    <div className="max-w-7xl mx-auto px-5 sm:px-6 py-12 space-y-14 text-slate-800 dark:text-slate-100 text-left">
      ...
      <div className="text-center max-w-2xl mx-auto space-y-2">
        ...
        <p className="text-slate-500 dark:text-slate-400 font-light text-sm">
          Dedicated to excellence in learning since 2006 — Sri Vani Vidyanikethan, Kottala.
        </p>
      ...
      <ScrollReveal>
        <div className="bg-slate-900 text-white rounded-3xl p-7 sm:p-10 shadow-xl border border-slate-800">
          ...
          <div className="grid sm:grid-cols-3 gap-6 text-center">
            {[
              { val: '#1', title: 'Best School in Kottala', desc: 'Ranked #1 for academic excellence and student welfare in the region.' },
          ...
          <div className="space-y-6">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
              {teachers.map((teacher) => {
                // Count marks entries for this teacher's subject
                const subjectMarksCount = (marks || []).filter(
                  m => m.subject?.toUpperCase() === teacher.subject?.toUpperCase()
                ).length;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="w-full border-t border-slate-100 dark:border-slate-800 mt-3 pt-3 space-y-1 text-[10px] text-slate-500 dark:text-slate-400">
                      ...
                      {subjectMarksCount > 0 && (
                        <div className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-bold rounded-lg px-2 py-1 mt-1">
                          📊 {subjectMarksCount} marks entries
                        </div>
                      )}
                    </div>
                  </div>
