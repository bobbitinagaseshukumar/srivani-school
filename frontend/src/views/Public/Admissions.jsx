...
export default function Admissions() {
  const { requiredDocuments, submitAdmissionApplication, fees } = useContext(AppContext);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.studentName || !formData.parentEmail || !formData.parentPhone || !formData.whatsappNumber) {
      alert('Please fill in student name, parent email, phone, and WhatsApp number.');
      return;
    }
          ...
          {submitted ? (
            <div className="text-center py-10 space-y-4 max-w-md mx-auto">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 font-light leading-relaxed">
                Thank you for submitting the registration. Your application has been sent to the Super Admin for review. You will receive a WhatsApp message on <strong>{formData.whatsappNumber}</strong> once it is approved.
              </p>
              <button
                onClick={() => {
                  setSubmitted(false);
                  setFormData({ studentName: '', dob: '', gender: '', grade: 'Class 10', parentName: '', parentEmail: '', parentPhone: '', whatsappNumber: '', address: '', prevSchool: '' });
                }}
