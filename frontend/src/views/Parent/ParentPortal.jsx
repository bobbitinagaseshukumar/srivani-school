...
import { sendEmail, buildFeePaymentReceiptEmail } from '../../lib/emailService';
...
export default function ParentPortal() {
  ...
  const handleCheckoutSubmit = (e) => {
    ...
    setTimeout(async () => {
      ...
      try {
        const feeBreakdown = [
          { description: 'Tuition & Term Fees', amount: `${tuitionAmount}.00` },
          { description: 'Laboratory / IT Fees', amount: `${labAmount}.00` },
          { description: 'School Bus / Transport', amount: `${busAmount}.00` },
          { description: 'Textbooks & Learning Material', amount: `${booksAmount}.00` }
        ];

        await sendEmail(
          targetEmail,
          'Sri Vani Vidyanikethan - Fee Payment Receipt',
          buildFeePaymentReceiptEmail({
            parentName,
            studentName: childObj.name,
            studentId: childObj.registerNo,
            className: `${childObj.class} - ${childObj.section}`,
            txnId,
            paymentDate,
            feeBreakdown,
            totalAmount: `${totalFeeAmount}.00`
          })
        );
      } catch (err) {
