...
export default function SuperAdminPortal() {
  const { 
    ...
    homepageInfrastructure, addHomepageInfrastructureItem, editHomepageInfrastructureItem, deleteHomepageInfrastructureItem,
    campusFacilities, addCampusFacility, editCampusFacility, deleteCampusFacility,
    requiredDocuments, addRequiredDocument, editRequiredDocument, deleteRequiredDocument,
    admissionApplications, approveAdmissionApplication, rejectAdmissionApplication,
    gradingInfo, updateGradingInfo,
    ...
    studentComplaints, updateStudentComplaintStatus, replyStudentComplaint,
    classes
  ...
  const systemMetrics = [
    ...
    { label: 'Pending Admissions', value: `${(admissionApplications || []).filter(a => a.status === 'Pending').length} Applications`, icon: Clipboard, color: 'text-amber-500' },
    { label: 'Student Complaints', value: `${(studentComplaints || []).filter(c => c.status === 'Pending').length} Open`, icon: AlertCircle, color: 'text-red-500' }
  ];
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8 text-slate-800 dark:text-slate-100 text-left font-sans">
      ...
      <div className="flex flex-wrap gap-1.5 border-b border-slate-200/50 dark:border-slate-800 pb-3 mb-6 overflow-x-auto">
        {['Schools','Admissions','Homepage','Facilities','Admins','Teachers','Students','Parents','Timetables','Fee Manager','News Ticker','Gallery Manager','Academics','Subjects','Complaints','Enquiries','Testimonials','School Settings','Audit Logs'].map((tab) => (
          <button
            key={tab}
          ...
          >
            ...
            )}{tab === 'Admissions' && (admissionApplications || []).filter(a => a.status === 'Pending').length > 0 && (
              <span className="ml-1 bg-amber-500 text-white text-[8px] font-black px-1.5 py-0.5 rounded-full">{(admissionApplications || []).filter(a => a.status === 'Pending').length}</span>
            )}{tab === 'Complaints' && (studentComplaints || []).filter(c => c.status === 'Pending').length > 0 && (
              <span className="ml-1 bg-red-500 text-white text-[8px] font-black px-1.5 py-0.5 rounded-full">{(studentComplaints || []).filter(c => c.status === 'Pending').length}</span>
            )}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'Admissions' && (
        <AdmissionsManagerPanel
          admissionApplications={admissionApplications}
          approveAdmissionApplication={approveAdmissionApplication}
          rejectAdmissionApplication={rejectAdmissionApplication}
          requiredDocuments={requiredDocuments}
          addRequiredDocument={addRequiredDocument}
          editRequiredDocument={editRequiredDocument}
          deleteRequiredDocument={deleteRequiredDocument}
        />
      )}

      {activeTab === 'Homepage' && (
        <HomepageManagerPanel
          homepageStats={homepageStats}
          updateHomepageStat={updateHomepageStat}
          homepageInfrastructure={homepageInfrastructure}
          addHomepageInfrastructureItem={addHomepageInfrastructureItem}
          editHomepageInfrastructureItem={editHomepageInfrastructureItem}
          deleteHomepageInfrastructureItem={deleteHomepageInfrastructureItem}
        />
      )}

      {activeTab === 'Facilities' && (
        <CampusFacilitiesManagerPanel
          campusFacilities={campusFacilities}
          addCampusFacility={addCampusFacility}
          editCampusFacility={editCampusFacility}
          deleteCampusFacility={deleteCampusFacility}
        />
      )}
        ...
        <div className="space-y-6">
          ...
          {showTeacherForm && (
            <form onSubmit={handleTeacherSubmit} className="glassmorphism p-5 rounded-2xl border border-white/50 shadow-lg space-y-4 max-w-2xl">
              ...
              <div className="grid grid-cols-2 gap-4">
                <input
                  type="text" required placeholder="Experience (e.g. 12 Years)" value={teacherForm.experience}
                  onChange={(e) => setTeacherForm(prev => ({ ...prev, experience: e.target.value }))}
                  className="px-3 py-2 border rounded-xl bg-white/70 dark:bg-slate-900/50 text-xs focus:ring-1 focus:ring-blue-500"
                />
                <input
                  type="text" placeholder="Designation" value={teacherForm.designation}
      ...
      {activeTab === 'Students' && (
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-3 items-center">
            ...
            />
            <select value={studentClassFilter} onChange={(e) => setStudentClassFilter(e.target.value)} className="px-3 py-2 border rounded-xl bg-white/70 dark:bg-slate-900/50 text-xs">
              ...
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-850 font-light">
                  {students
                    ...
                    .filter(s => s.name.toLowerCase().includes(studentSearch.toLowerCase()) || s.registerNo.toLowerCase().includes(studentSearch.toLowerCase())).map((stud) => (
      ...
      {activeTab === 'Academics' && (
        <AcademicsManagerPanel
          ...
          updateGradingInfo={updateGradingInfo}
        />
      )}
      ...
      {activeTab === 'Complaints' && (
        <StudentComplaintsPanel
          ...
          updateStudentComplaintStatus={updateStudentComplaintStatus}
          replyStudentComplaint={replyStudentComplaint}
        />
      )}
...
/* ============================================================
   ...
   ============================================================ */
const GALLERY_CATEGORIES_DEFAULT = ['Sports', 'Annual Day', 'Special Events', 'Cultural Programs', 'Science Exhibition', 'Classroom Activities', 'Independence Day', 'Republic Day', 'Teachers Day', 'Other Events', 'Other'];
...
function GalleryManagerPanel({ galleryItems, addGalleryItem, editGalleryItem, deleteGalleryItem, galleryCategories, addGalleryCategory, editGalleryCategory, deleteGalleryCategory }) {
  ...
  const [form, setForm] = useState({
    title: '', category: (galleryCategories && galleryCategories[0]) || 'Sports', description: '',
    date: new Date().toISOString().split('T')[0],
    type: 'image', url: ''
  });
  const [newCategoryName, setNewCategoryName] = React.useState('');
  const categoryList = galleryCategories && galleryCategories.length > 0 ? galleryCategories : GALLERY_CATEGORIES_DEFAULT;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glassmorphism p-4 rounded-2xl border border-white/40 space-y-3">
        <h4 className="font-bold text-xs uppercase text-slate-500">Manage Categories</h4>
        <div className="flex flex-wrap gap-2">
          {categoryList.map(cat => (
            <span key={cat} className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-slate-100 dark:bg-slate-900 text-slate-600">{cat}</span>
          ))}
        </div>
        <div className="flex gap-2 max-w-md">
          <input value={newCategoryName} onChange={e => setNewCategoryName(e.target.value)} placeholder="New category name" className="flex-1 px-3 py-2 border rounded-xl text-xs" />
          <button onClick={() => { if (addGalleryCategory(newCategoryName)) setNewCategoryName(''); }} className="bg-emerald-600 text-white text-xs font-bold px-3 py-2 rounded-xl">Add</button>
        </div>
      </div>
...
function AcademicsManagerPanel({
  academicCalendar, addCalendarEvent, editCalendarEvent, deleteCalendarEvent,
  academicPrograms, addAcademicProgram, editAcademicProgram, deleteAcademicProgram,
  subjects, gradingInfo, updateGradingInfo
}) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        {[['calendar', '📅 Academic Calendar'], ['programs', '📚 Program Levels'], ['grading', '🏆 Grading Process']].map(([key, label]) => (
      ...
      {section === 'grading' && (
        <div className="space-y-4 max-w-2xl">
          <p className="text-xs text-slate-500">Controls the Result & Grading Process section on the public Academics page.</p>
          <div className="bg-white dark:bg-slate-800/80 rounded-2xl border p-5 space-y-4">
            <div>
              <label className="text-[10px] font-bold uppercase text-slate-500">Assessment Title</label>
              <input className="w-full mt-1 px-3 py-2 border rounded-xl text-xs" value={gradingInfo?.assessmentTitle || ''} onChange={e => updateGradingInfo({ assessmentTitle: e.target.value })} />
            </div>
            <div>
              <label className="text-[10px] font-bold uppercase text-slate-500">Assessment Description</label>
              <textarea rows={3} className="w-full mt-1 px-3 py-2 border rounded-xl text-xs" value={gradingInfo?.assessmentDescription || ''} onChange={e => updateGradingInfo({ assessmentDescription: e.target.value })} />
            </div>
            <div>
              <label className="text-[10px] font-bold uppercase text-slate-500">Grading Scheme (JSON rows: grade, range, color)</label>
              {(gradingInfo?.scheme || []).map((row, idx) => (
                <div key={idx} className="grid grid-cols-3 gap-2 mt-2">
                  <input className="px-2 py-1 border rounded text-xs" value={row.grade} onChange={e => {
                    const scheme = [...(gradingInfo.scheme || [])];
                    scheme[idx] = { ...scheme[idx], grade: e.target.value };
                    updateGradingInfo({ scheme });
                  }} />
                  <input className="px-2 py-1 border rounded text-xs" value={row.range} onChange={e => {
                    const scheme = [...(gradingInfo.scheme || [])];
                    scheme[idx] = { ...scheme[idx], range: e.target.value };
                    updateGradingInfo({ scheme });
                  }} />
                  <input className="px-2 py-1 border rounded text-xs" value={row.color} onChange={e => {
                    const scheme = [...(gradingInfo.scheme || [])];
                    scheme[idx] = { ...scheme[idx], color: e.target.value };
                    updateGradingInfo({ scheme });
                  }} />
                </div>
              ))}
            ...
            <div>
              <label className="text-[10px] font-bold uppercase text-slate-500">Tip Text</label>
              <input className="w-full mt-1 px-3 py-2 border rounded-xl text-xs" value={gradingInfo?.tipText || ''} onChange={e => updateGradingInfo({ tipText: e.target.value })} />
            </div>
          </div>
        </div>
      )}
...
function SubjectsManagerPanel({ subjects, addSubject, editSubject, deleteSubject, departments, addDepartment, editDepartment, deleteDepartment }) {
  ...
  const [deptEditId, setDeptEditId] = React.useState(null);
  const [deptEditName, setDeptEditName] = React.useState('');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glassmorphism p-4 rounded-2xl border border-white/40 space-y-3">
        <h4 className="font-bold text-sm">Departments Manager</h4>
        <div className="flex flex-wrap gap-2">
          {(departments || []).map(d => (
            <div key={d.id} className="flex items-center gap-1 bg-slate-100 dark:bg-slate-900 rounded-full px-2 py-1">
              {deptEditId === d.id ? (
                <>
                  <input value={deptEditName} onChange={e => setDeptEditName(e.target.value)} className="text-[10px] px-2 py-0.5 rounded bg-white dark:bg-slate-800 w-24" />
                  <button onClick={() => { editDepartment(d.id, deptEditName); setDeptEditId(null); }} className="text-[10px] text-emerald-600 font-bold">Save</button>
                </>
              ) : (
                <>
                  <span className="text-[10px] font-bold">{d.name}</span>
                  <button onClick={() => { setDeptEditId(d.id); setDeptEditName(d.name); }} className="text-[10px] text-blue-600">Edit</button>
                  <button onClick={() => { const r = deleteDepartment(d.id); if (r && !r.success) alert(r.message); }} className="text-[10px] text-red-500">Del</button>
                </>
              )}
            </div>
          ))}
        ...
        <div className="flex gap-2 max-w-sm">
          <input value={deptName} onChange={e => setDeptName(e.target.value)} placeholder="New department" className="flex-1 px-3 py-2 border rounded-xl text-xs" />
          <button onClick={() => { const r = addDepartment(deptName); if (r.success) setDeptName(''); else if (r.message) alert(r.message); }} className="bg-emerald-600 text-white text-xs font-bold px-3 py-2 rounded-xl">Add Dept</button>
        </div>
      </div>
...
function AdmissionsManagerPanel({ admissionApplications, approveAdmissionApplication, rejectAdmissionApplication, requiredDocuments, addRequiredDocument, editRequiredDocument, deleteRequiredDocument }) {
  const [docText, setDocText] = React.useState('');
  const [editDocIdx, setEditDocIdx] = React.useState(null);
  const [editDocText, setEditDocText] = React.useState('');

  return (
    <div className="space-y-8">
      <div>
        <h3 className="font-extrabold text-lg font-montserrat">Online Admission Applications</h3>
        <p className="text-xs text-slate-500 mt-1">Review applications submitted from the public Admissions page. Approve to send WhatsApp notification.</p>
      </div>
      <div className="space-y-3">
        {(admissionApplications || []).length === 0 && <p className="text-sm text-slate-400">No applications yet.</p>}
        {(admissionApplications || []).map(app => (
          <div key={app.id} className="bg-white dark:bg-slate-800/60 rounded-2xl border p-4 space-y-2 text-xs">
            <div className="flex justify-between items-start gap-2">
              <div>
                <p className="font-bold text-sm">{app.studentName} → {app.grade}</p>
                <p className="text-slate-500">Parent: {app.parentName} | WhatsApp: {app.whatsappNumber || app.parentPhone}</p>
                <p className="text-slate-400">Submitted: {app.submittedAt}</p>
              </div>
              <span className={`text-[9px] font-bold px-2 py-0.5 rounded ${app.status === 'Approved' ? 'bg-emerald-500/10 text-emerald-600' : app.status === 'Rejected' ? 'bg-red-500/10 text-red-500' : 'bg-amber-500/10 text-amber-600'}`}>{app.status}</span>
            </div>
            {app.status === 'Pending' && (
              <div className="flex gap-2 pt-2">
                <button onClick={() => { const r = approveAdmissionApplication(app.id); if (r.success) alert(`WhatsApp approval message sent to ${r.whatsappNumber}`); }} className="bg-emerald-600 text-white font-bold px-3 py-1.5 rounded-lg">Approve & WhatsApp</button>
                <button onClick={() => rejectAdmissionApplication(app.id)} className="bg-red-500 text-white font-bold px-3 py-1.5 rounded-lg">Reject</button>
              </div>
            )}
            {app.whatsappSent && <p className="text-[10px] text-emerald-600">✓ WhatsApp sent at {app.whatsappSentAt}</p>}
          </div>
        ))}
      </div>

      <div className="space-y-4 border-t pt-6">
        <h3 className="font-extrabold text-lg font-montserrat">Required Documents (Admissions Page)</h3>
        <ul className="space-y-2">
          {(requiredDocuments || []).map((doc, idx) => (
            <li key={idx} className="flex items-center gap-2 text-xs bg-slate-50 dark:bg-slate-900/40 p-2 rounded-lg">
              {editDocIdx === idx ? (
                <>
                  <input value={editDocText} onChange={e => setEditDocText(e.target.value)} className="flex-1 px-2 py-1 border rounded text-xs" />
                  <button onClick={() => { editRequiredDocument(idx, editDocText); setEditDocIdx(null); }} className="text-emerald-600 font-bold">Save</button>
                </>
              ) : (
                <>
                  <span className="flex-1">{doc}</span>
                  <button onClick={() => { setEditDocIdx(idx); setEditDocText(doc); }} className="text-blue-600">Edit</button>
                  <button onClick={() => deleteRequiredDocument(idx)} className="text-red-500">Delete</button>
                </>
              )}
            </li>
          ))}
        ...
        <div className="flex gap-2 max-w-lg">
          <input value={docText} onChange={e => setDocText(e.target.value)} placeholder="Add required document..." className="flex-1 px-3 py-2 border rounded-xl text-xs" />
          <button onClick={() => { if (docText.trim()) { addRequiredDocument(docText.trim()); setDocText(''); } }} className="bg-blue-600 text-white text-xs font-bold px-4 py-2 rounded-xl">Add</button>
        </div>
      </div>
    </div>
  );
...
function HomepageManagerPanel({ homepageStats, updateHomepageStat, homepageInfrastructure, addHomepageInfrastructureItem, editHomepageInfrastructureItem, deleteHomepageInfrastructureItem }) {
  const [infraForm, setInfraForm] = React.useState(null);

  return (
    <div className="space-y-8">
      <div>
        <h3 className="font-extrabold text-lg font-montserrat">Homepage Statistics</h3>
        <div className="grid sm:grid-cols-2 gap-4 mt-4">
          {(homepageStats || []).map(stat => (
            <div key={stat.id} className="glassmorphism p-4 rounded-2xl border space-y-2 text-xs">
              <input value={stat.label} onChange={e => updateHomepageStat(stat.id, { label: e.target.value })} className="w-full px-2 py-1 border rounded" placeholder="Label" />
              <input value={stat.count} onChange={e => updateHomepageStat(stat.id, { count: e.target.value })} className="w-full px-2 py-1 border rounded" placeholder="Value" />
            </div>
          ))}
        </div>
      </div>
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={() => setInfraForm({ title: '', desc: '', img: '' })} className="bg-blue-600 text-white text-xs font-bold px-3 py-2 rounded-xl">Add Item</button>
        </div>
        {infraForm && (
          <div className="glassmorphism p-4 rounded-2xl border space-y-2 mb-4 text-xs">
            <input placeholder="Title" value={infraForm.title} onChange={e => setInfraForm(f => ({ ...f, title: e.target.value }))} className="w-full px-2 py-1 border rounded" />
            <textarea placeholder="Description" value={infraForm.desc} onChange={e => setInfraForm(f => ({ ...f, desc: e.target.value }))} className="w-full px-2 py-1 border rounded" rows={2} />
            <input placeholder="Image URL" value={infraForm.img} onChange={e => setInfraForm(f => ({ ...f, img: e.target.value }))} className="w-full px-2 py-1 border rounded" />
            <div className="flex gap-2">
              <button onClick={() => { if (infraForm.id) editHomepageInfrastructureItem(infraForm.id, infraForm); else addHomepageInfrastructureItem(infraForm); setInfraForm(null); }} className="bg-emerald-600 text-white px-3 py-1.5 rounded-lg font-bold">Save</button>
              <button onClick={() => setInfraForm(null)} className="px-3 py-1.5 rounded-lg bg-slate-100">Cancel</button>
            </div>
          </div>
        )}
        <div className="grid sm:grid-cols-2 gap-4">
          {(homepageInfrastructure || []).map(item => (
            <div key={item.id} className="bg-white dark:bg-slate-800/60 rounded-xl border p-3 space-y-2 text-xs">
              <img src={item.img} alt={item.title} className="w-full h-28 object-cover rounded-lg" />
              <p className="font-bold">{item.title}</p>
              <p className="text-slate-500">{item.desc}</p>
              <div className="flex gap-2">
                <button onClick={() => setInfraForm({ ...item })} className="text-blue-600 font-bold">Edit</button>
                <button onClick={() => deleteHomepageInfrastructureItem(item.id)} className="text-red-500 font-bold">Delete</button>
              </div>
            </div>
          ))}
        </div>
...
function CampusFacilitiesManagerPanel({ campusFacilities, addCampusFacility, editCampusFacility, deleteCampusFacility }) {
  const [form, setForm] = React.useState(null);
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="font-extrabold text-lg font-montserrat">Campus Facilities Page</h3>
        <button onClick={() => setForm({ title: '', icon: 'Book', cap: '', desc: '', features: '' })} className="bg-blue-600 text-white text-xs font-bold px-3 py-2 rounded-xl">Add Facility</button>
      </div>
      {form && (
        <div className="glassmorphism p-4 rounded-2xl border space-y-2 text-xs max-w-xl">
          <input placeholder="Title" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} className="w-full px-2 py-1 border rounded" />
          <input placeholder="Icon (Book, Monitor, Cpu, Award, Navigation, Home)" value={form.icon} onChange={e => setForm(f => ({ ...f, icon: e.target.value }))} className="w-full px-2 py-1 border rounded" />
          <input placeholder="Capacity" value={form.cap} onChange={e => setForm(f => ({ ...f, cap: e.target.value }))} className="w-full px-2 py-1 border rounded" />
          <textarea placeholder="Description" value={form.desc} onChange={e => setForm(f => ({ ...f, desc: e.target.value }))} className="w-full px-2 py-1 border rounded" rows={2} />
          <input placeholder="Features (comma separated)" value={form.features} onChange={e => setForm(f => ({ ...f, features: e.target.value }))} className="w-full px-2 py-1 border rounded" />
          <button onClick={() => {
            const data = { ...form, features: (typeof form.features === 'string' ? form.features.split(',').map(s => s.trim()).filter(Boolean) : form.features) };
            if (form.id) editCampusFacility(form.id, data); else addCampusFacility(data);
            setForm(null);
          }} className="bg-emerald-600 text-white font-bold px-4 py-2 rounded-xl">Save</button>
        </div>
      )}
      <div className="grid sm:grid-cols-2 gap-4">
        {(campusFacilities || []).map(fac => (
          <div key={fac.id} className="bg-white dark:bg-slate-800/60 rounded-xl border p-4 text-xs space-y-1">
            <p className="font-bold">{fac.title}</p>
            <p className="text-slate-500">{fac.desc}</p>
            <div className="flex gap-2 pt-2">
              <button onClick={() => setForm({ ...fac, features: (fac.features || []).join(', ') })} className="text-blue-600 font-bold">Edit</button>
              <button onClick={() => deleteCampusFacility(fac.id)} className="text-red-500 font-bold">Delete</button>
            </div>
          </div>
        ))}
      </div>
...
function StudentComplaintsPanel({ studentComplaints, updateStudentComplaintStatus, replyStudentComplaint }) {
  const [replyMap, setReplyMap] = React.useState({});
  return (
    <div className="space-y-6">
      <h3 className="font-extrabold text-lg font-montserrat">Student Complaint Box</h3>
      {(studentComplaints || []).length === 0 && <p className="text-sm text-slate-400">No complaints received yet.</p>}
      {(studentComplaints || []).map(c => (
        <div key={c.id} className="bg-white dark:bg-slate-800/60 rounded-2xl border p-4 space-y-3 text-xs">
          <div className="flex justify-between items-start">
            <div>
              <p className="font-bold text-sm">{c.subject}</p>
              <p className="text-slate-500">{c.studentName} | {c.class} {c.section} | Reg: {c.registerNo}</p>
              <p className="text-slate-400">Contact: {c.contactPhone} | {c.contactEmail}</p>
              <p className="mt-2">{c.description}</p>
              <p className="text-[9px] text-slate-400 mt-1">Submitted: {c.submittedAt}</p>
            </div>
            <select value={c.status} onChange={e => updateStudentComplaintStatus(c.id, e.target.value)} className="text-[10px] border rounded px-2 py-1">
              <option>Pending</option>
              <option>In Progress</option>
              <option>Resolved</option>
            </select>
          </div>
          {c.adminReply && <div className="bg-blue-500/5 p-2 rounded"><strong>Reply:</strong> {c.adminReply}</div>}
          <div className="flex gap-2">
            <input value={replyMap[c.id] || ''} onChange={e => setReplyMap(m => ({ ...m, [c.id]: e.target.value }))} placeholder="Reply to student..." className="flex-1 px-2 py-1 border rounded" />
            <button onClick={() => { replyStudentComplaint(c.id, replyMap[c.id] || ''); setReplyMap(m => ({ ...m, [c.id]: '' })); }} className="bg-blue-600 text-white font-bold px-3 py-1 rounded-lg">Send Reply</button>
          </div>
        </div>
      ))}
    </div>
